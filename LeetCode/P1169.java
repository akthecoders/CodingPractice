...
public class P1169 {
    ...
    public List<String> invalidTransactions(String[] transactions) {
        ...
        for (int i = 0; i < transactions.length; ++i) {
            ...
            TransactionEntry entry = new TransactionEntry(i, splittedTransaction[0],
                    Integer.parseInt(splittedTransaction[1]), Integer.parseInt(splittedTransaction[2]),
    ...
    private void checkList(List<TransactionEntry> entries, List<String> invalidTransactions, String[] transactions) {
        for (TransactionEntry entry1 : entries) {
            ...
            for (TransactionEntry entry2 : entries) {
                if (entry1.id != entry2.id && Math.abs(entry1.time - entry2.time) <= MAX_DIFF
                        && !entry1.city.equals(entry2.city)) {
                    invalidTransactions.add(transactions[entry1.id]);
                    break;
                }
            }
